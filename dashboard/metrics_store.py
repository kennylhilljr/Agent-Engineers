...
# Canonical list of all agents in the system.
# These always appear on the dashboard, even with zero activity.
...
def _empty_agent_profile(name: str) -> AgentProfile:
    """Create a zeroed-out AgentProfile for an agent with no activity yet."""
    return {
        "agent_name": name,
        "total_invocations": 0,
...
class MetricsStore:
    ...
    def _ensure_all_agents(self, state: DashboardState) -> DashboardState:
        """Ensure every canonical agent has an entry in state['agents'].

        Agents with no activity get a zeroed-out profile so they always
        appear on the dashboard.
        """
        for name in ALL_AGENT_NAMES:
            if name not in state["agents"]:
                state["agents"][name] = _empty_agent_profile(name)
        return state
        ...
        with self._thread_lock:
            with _file_lock(self.lock_path, self.LOCK_TIMEOUT):
                try:
                    ...
                    if self.metrics_path.exists():
                        try:
                            ...
                            if self._validate_state(data):
                                return self._ensure_all_agents(data)  # type: ignore
                            else:
                    ...
                    return self._ensure_all_agents(self._create_empty_state())
