...
/**
 * TaskCard - Displays task information with status, assignee, and priority
 * Status: Placeholder - Full implementation pending
 */
export function TaskCard({ type, data, metadata }: A2UIProps) {
  ...
  return (
    <div className="border rounded-lg p-4 bg-card text-card-foreground">
      ...
      <pre className="text-xs bg-muted p-2 rounded overflow-auto max-h-48">
        {JSON.stringify({ data: taskData, metadata }, null, 2)}
      </pre>
    </div>
  );
}
